...
df = pd.read_csv('/content/SOL_USD_1h_2y.csv')
df.head()

df = pd.read_csv('/content/SOL_USD_1h_2y.csv',header=0, skiprows=[1,2])
...
df['sma_13'] = calculate_sma(df,window=13)
...
nw_bands = calculate_nadaraya_watson_envelope(
    df,
    window=20,
    bandwidth=3,
    deviation_window=20,
...
df[['close','bb_lower','bb_middle','bb_upper','dc_lower_20','dc_middle_20','dc_upper_20',
    'sma_13', 'sma_50', 'sma_200', 'trend_13_50', 'trend_50_200','supertrend',
       'supertrend_direction', 'supertrend_signal']].tail(50)
...
df['dc_position'] = np.where(df['close'] > df['dc_middle'], 'above', 'below')
df['dc_breakout'] = df['close'] > df['dc_upper']
df['dc_breakdown'] = df['close'] < df['dc_lower']

"""## SMA Cross"""

df['sma_cross_13_50'] = df['trend_13_50'] != df['trend_13_50'].shift(1)
df['sma_cross_50-200'] = df['trend_50_200'] != df['trend_50_200'].shift(1)
